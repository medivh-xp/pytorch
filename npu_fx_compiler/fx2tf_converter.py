from concrete_graph import ConcreteGraphBase, ValuePack
...
class TfConcreteGraph(ConcreteGraphBase):
    ...
    def parse_input(self, target: 'Target', args: Tuple[Argument, ...], kwargs: Dict[str, Any], meta_outputs: Any):
        if isinstance(meta_outputs, torch.SymInt):
            ...
            self._inputs.append(tf.placeholder(name=target, dtype=_torch_type_to_tf_type(meta_outputs.dtype)))
    ...
    def parse_output(self, target: 'Target', args: Tuple[Argument, ...], kwargs: Dict[str, Any], meta_outputs: Any):
        ...
        for arg in args:
            ...
            if isinstance(arg, tf.Tensor):
                self._fx_outputs_mapping[len(self._fx_outputs)] = len(self._outputs)
    ...
    def __call__(self, *args: Any, **kwds: Any) -> Any:
        with tf.Session(graph=self.graph) as sess:
            ...
            tf_outputs = sess.run(self.outputs, feed_dict=dict(zip(self.inputs, args)))
